...
class BybitWebSocketManager:
    ...
    def __init__(self, credentials: BybitCredentials, endpoints: BybitEndpoints):
        ...
        self._websockets_initialized = False
        ...
        # ИСПРАВЛЕНИЕ: Инициализация флага heartbeat
    ...
    def get_connection_status(self) -> Dict[str, Any]:
        ...
        status = {
            'private': {
                'connected': 'private' in self.connections and self.connections['private'],
                'reconnect_attempts': self.reconnect_attempts.get('private', 0),
                'last_message': self.last_message_time.get('private', 0)
            },
            'public': {
                'connected': 'public' in self.connections and self.connections['public'],
                'reconnect_attempts': self.reconnect_attempts.get('public', 0),
                'last_message': self.last_message_time.get('public', 0)
            }
        }
        return status
    
    def reset_reconnect_attempts(self, ws_type: str = None):
        """Сброс счетчика попыток переподключения"""
        if ws_type:
            self.reconnect_attempts[ws_type] = 0
            logger.info(f"✅ Сброшен счетчик попыток для {ws_type}")
        else:
            self.reconnect_attempts = {}
            logger.info("✅ Сброшены все счетчики попыток")
    ...
    def connect_private(self, callback: Callable):
        ...
        try:
            # ВАЖНО: Проверяем, не подключены ли уже
            if 'private' in self.connections and self.connections['private']:
                logger.warning("⚠️ Приватный WebSocket уже подключен, пропускаем")
                return self.connections['private']
            # Сохраняем ссылку на менеджер для доступа из вложенных функций
            manager = self
            
            # Проверяем нужно ли запускать heartbeat
            heartbeat_needed = not self._heartbeat_started
            ...
            def on_message(ws, message):
                try:
                    ...
                    if data.get('op') == 'ping':
                        ...
                        logger.debug("🏓 Pong отправлен на private")
                        return
            ...
            def on_error(ws, error):
                ...
                if "403" in error_str or "too many requests" in error_str.lower():
                    logger.warning("⚠️ Rate limiting обнаружен! Увеличиваем задержку переподключения")
                    # Увеличиваем счетчик попыток для большей задержки
                    current_attempts = manager.reconnect_attempts.get('private', 0)
                    manager.reconnect_attempts['private'] = min(current_attempts + 3, manager.max_reconnect_attempts - 1)
            ...
            def on_open(ws):
                ...
                manager.last_message_time['private'] = time.time()
                # Отправляем аутентификацию
                auth_payload = manager._create_auth_payload()
            ...
            if heartbeat_needed:
                ...
                self._heartbeat_started = True  # Устанавливаем флаг
                logger.info("💓 WebSocket heartbeat запущен")
    ...
    def connect_public(self, callback: Callable):
        ...
        try:
            # ВАЖНО: Проверяем, не подключены ли уже
            if 'public' in self.connections and self.connections['public']:
                logger.warning("⚠️ Публичный WebSocket уже подключен, пропускаем")
            ...
            def on_message(ws, message):
                try:
                    ...
                    if data.get('op') == 'ping':
                        pong_msg = {'op': 'pong', 'args': [str(int(time.time() * 1000))]}
                        ws.send(json.dumps(pong_msg))
                        logger.debug("🏓 Pong отправлен на public")
                        return
            ...
            def on_error(ws, error):
                ...
                if "403" in error_str or "too many requests" in error_str.lower():
                    logger.warning("⚠️ Rate limiting обнаружен! Увеличиваем задержку переподключения")
                    # Увеличиваем счетчик попыток для большей задержки
                    current_attempts = manager.reconnect_attempts.get('public', 0)
                    manager.reconnect_attempts['public'] = min(current_attempts + 3, manager.max_reconnect_attempts - 1)
    ...
    def _schedule_reconnect(self, ws_type: str):
        ...
        # Проверяем, не запланировано ли уже переподключение
        reconnect_key = f"{ws_type}_reconnect_scheduled"
        if hasattr(self, reconnect_key) and getattr(self, reconnect_key):
            logger.info(f"⏳ Переподключение {ws_type} уже запланировано")
            return
        ...
        # Добавляем случайный джиттер для избежания синхронных переподключений
        jitter = random.uniform(0, delay * 0.1)
        total_delay = delay + jitter
        
        logger.info(f"🔄 Переподключение {ws_type} через {total_delay:.1f}s (попытка {attempts + 1}/{self.max_reconnect_attempts})")
        
        # Устанавливаем флаг, что переподключение запланировано
        setattr(self, reconnect_key, True)
        
        def delayed_reconnect():
            try:
                # Сбрасываем флаг
                setattr(self, reconnect_key, False)
    ...
    def _reconnect(self, ws_type: str):
        """Переподключение WebSocket - ИСПРАВЛЕННАЯ ВЕРСИЯ"""
        try:
            ...
            # Создаем новое соединение в новом потоке с новым event loop
            def reconnect_in_new_loop():
                try:
                    ...
                    loop.run_until_complete(self._async_reconnect(ws_type))
            ...
            reconnect_thread = threading.Thread(
                target=reconnect_in_new_loop,
                daemon=True,
                name=f"ws_reconnect_{ws_type}"
            )
    ...
    async def _async_reconnect(self, ws_type: str):
        """Асинхронное переподключение"""
        try:
            ...
            # Проверяем статус
            if ws_type in self.connections:
                logger.info(f"✅ {ws_type} WebSocket успешно переподключен")
                # Сбрасываем счетчик попыток
                self.reconnect_attempts[ws_type] = 0
            else:
                logger.warning(f"⚠️ {ws_type} WebSocket не подключен, повторная попытка...")
                self._schedule_reconnect(ws_type)
                
        except Exception as e:
            logger.error(f"❌ Ошибка асинхронного переподключения: {e}")
            self._schedule_reconnect(ws_type)
        ...
        try:
            ...
            if hasattr(self, 'heartbeat_task') and self.heartbeat_task and not self.heartbeat_task.done():
                ...
                try:
                    ...
                    pass
                logger.info("💔 WebSocket heartbeat остановлен")
                self._heartbeat_started = False  # Сбрасываем флаг
...
class BybitClientV5:
        ...
        try:
            ...
            try:
                self.exchange = ccxt.bybit({
                    ...
                    'rateLimit': 3000,  # Увеличено с 2000 до 3000
                    'timeout': 30000,
                    'options': {
                        'defaultType': 'linear',
                        ...
                        'fetchOrderBookRateLimit': 100,  # Лимит для orderbook
                        'fetchTickerRateLimit': 50,       # Лимит для ticker
                        'fetchOHLCVRateLimit': 100        # Лимит для свечей
                    }
                })
    ...
    def start_private_websocket(self, callback: Callable):
        ...
        if 'private' in self.ws_manager.connections and self.ws_manager.connections['private']:
            logger.info("ℹ️ Приватный WebSocket уже запущен")
            return self.ws_manager.connections['private']
    ...
    def start_public_websocket(self, callback: Callable):
        ...
        # Проверяем, не запущен ли уже
        if 'public' in self.ws_manager.connections and self.ws_manager.connections['public']:
            logger.info("ℹ️ Публичный WebSocket уже запущен")
            return self.ws_manager.connections['public']
