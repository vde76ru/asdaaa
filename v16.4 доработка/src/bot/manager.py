        ...
        try:
            ...
            if hasattr(self, 'execution_engine') and self.execution_engine and hasattr(self.execution_engine, 'execute_signal'):
                try:
                    ...
                    # ИСПРАВЛЕНО: Создаем сигнал БЕЗ параметра symbol
                    from ..strategies.base import TradingSignal
                    trading_signal = TradingSignal(
                        action=signal.upper(),  # Первый позиционный параметр
                        confidence=trade_data.get('confidence', 0.6),  # Второй позиционный
                        price=price,  # Третий позиционный
                    ...
                    if result and hasattr(result, 'status') and result.status.value == 'completed':
                        ...
                        logger.warning(f"⚠️ Execution_engine вернул статус: {result.status.value if result and hasattr(result, 'status') else 'None'}")
            ...
            if hasattr(self, 'exchange_client') and self.exchange_client and hasattr(self.exchange_client, 'place_order'):
                try:
                    ...
                    # Валидация stop_loss и take_profit перед отправкой
                    validated_sl = trade_data.get('stop_loss')
                    validated_tp = trade_data.get('take_profit')
                    
                    if validated_sl:
                        if signal.upper() == 'BUY' and validated_sl >= price:
                            validated_sl = price * 0.97  # 3% ниже
                            logger.warning(f"⚠️ Скорректирован stop_loss для BUY: {validated_sl}")
                        elif signal.upper() == 'SELL' and validated_sl <= price:
                            validated_sl = price * 1.03  # 3% выше
                            logger.warning(f"⚠️ Скорректирован stop_loss для SELL: {validated_sl}")
                    
                    # ИСПРАВЛЕНО: Правильный вызов place_order
            ...
            if hasattr(self, 'enhanced_exchange_client') and self.enhanced_exchange_client:
                try:
                    ...
                    if hasattr(self.enhanced_exchange_client, 'v5_client') and self.enhanced_exchange_client.v5_client:
                        # ИСПРАВЛЕНО: Используем V5 API правильно
                        ...
                        if order_result and order_result.get('retCode') == 0:
                            logger.info(f"✅ Ордер выполнен через V5 API: {order_result.get('result', {}).get('orderId', 'unknown')}")
                            return True
                            
                    # Проверяем метод place_order с сигналом
                    elif hasattr(self.enhanced_exchange_client, 'place_order'):
                        # ИСПРАВЛЕНО: Создаем правильный TradingSignal для enhanced client
                        from ..strategies.base import TradingSignal
                        trading_signal = TradingSignal(
                            action=signal.upper(),
                    ...
                    logger.warning("⚠️ Enhanced exchange не имеет подходящего метода для размещения ордера")
    ...
    async def _execute_best_trades(self, opportunities: list) -> int:
        ...
        try:
            ...
            for opportunity in sorted_opportunities[:max_trades]:
                ...
                # ИСПРАВЛЕНО: Проверяем режим торговли правильно
