...
class BybitWebSocketManager:
    ...
    async def _reconnect_websocket(self, ws_type: str = 'public'):
        """Улучшенное переподключение WebSocket с exponential backoff"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                if ws_type == 'public':
                    success = await self._init_public_websocket()
                else:
                    success = await self._init_private_websocket()
    ...
    def _generate_signature(self, timestamp: int, recv_window: int = 5000) -> str:
        """Генерация подписи для WebSocket аутентификации"""
        param_str = f"GET/realtime{recv_window}{timestamp}"
        return hmac.new(
